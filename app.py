...
from service_api import Base, make_app, session_scope
...
def run_app(port, host, config):
    """
    Starts flask aplication in development mode
    """
    flask_app = make_app(config)
    flask_app.run(port=port, host=host)
    # print(port, host, config)


@cli.command("run_celery")
def run_celery(port, host):
    os.system(f"celery -A service_api.celery_tasks.tasks.celery_app worker -B")


@cli.command("load_core_data")
@click.option("--column", "-C", "columns", is_flag=False, default=[], show_default=True,
